...
export default async function handler(req, res) {
  ...
  // action=track: proxy Andreani (consolidado desde andreani-tracking.js para
  // bajar el conteo de functions Vercel Hobby de 16 a 12).
  if (req.query.action === "track") {
    const { tracking: trk } = req.query;
    if (!trk) return res.status(400).json({ error: 'tracking requerido' });
    const headers = { 'Accept': 'application/json', 'User-Agent': 'GrowithApp (soluna.biolight@gmail.com)' };
    try {
      const r1 = await fetch(`https://api.andreani.com/v2/ordenes/${encodeURIComponent(trk)}`, { headers });
      if (r1.ok) { const d = await r1.json(); return res.status(200).json(d); }
    } catch(_) {}
    try {
      const r2 = await fetch(`https://tracking.andreani.com/api/v1/seguimiento?tracking=${encodeURIComponent(trk)}`, { headers });
