...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      ...
      return res.redirect(`https://soluna-gestion.vercel.app?tn_error=token_failed`);
    }
    ...
    if (!userSnap.exists) {
      return res.redirect(`https://soluna-gestion.vercel.app?tn_error=user_not_found`);
    }
    ...
    return res.redirect(`https://soluna-gestion.vercel.app?tn_success=1`);

  } catch (e) {
    console.error("TN callback error:", e);
    return res.redirect(`https://soluna-gestion.vercel.app?tn_error=server_error`);
  }
}
